...
export function registerPrivacyRoutes(app: Hono, db: Db, config: Config) {
  ...
  app.get("/:locale/privacy", (context) => { const locale = localeParam(context.req.param("locale")); if (!locale) return context.notFound(); rememberLocale(context, locale, config); const body = t(locale, "privacyBody").split("\n\n").filter(paragraph => config.privacyContactEmail || !paragraph.includes("{{PRIVACY_CONTACT_EMAIL}}")).join("\n\n").replaceAll("{{PRIVACY_CONTACT_EMAIL}}", config.privacyContactEmail); return context.html(<Layout locale={locale} title={t(locale, "privacyTitle")} path={context.req.path}><div class="privacy-page"><JourneyIntro title={t(locale, "privacyTitle")} /><Surface class="privacy-surface"><div class="privacy-copy">{privacyParagraphs(body)}<p><a href={`/${locale}/delete-data`}>{t(locale, "deleteTitle")}</a></p></div></Surface></div></Layout>); });
