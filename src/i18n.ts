...
export const locales = ["he", "ar", "yi", "ru", "uk", "en", "am"] as const;
...
export const localeNames: Record<Locale, string> = {
  he: "עברית", ar: "العربية", yi: "ייִדיש", ru: "Русский", uk: "Українська", en: "English", am: "አማርኛ"
};
...
const requestUi: Record<Locale, Record<string, string>> = {
  ...
  uk: { requestPreparedNote: "Текст нижче підготовлено для перевірки. Нічого не надсилається автоматично.", shareForRecipient: "Поділіться цим зверненням із" },
  am: { requestPreparedNote: "ጽሑፉ ለመገምገም ተዘጋጅቷል። ምንም ነገር በራስ-ሰር አይላክም።", shareForRecipient: "ይህን ጥያቄ ለዚህ ያጋሩ" }
};
const dictionaries: Record<Locale, Record<string, string>> = { am: { ...am, ...requestUi.am }, ar: { ...ar, ...requestUi.ar }, en: { ...en, ...requestUi.en }, he: { ...he, ...requestUi.he }, ru: { ...ru, ...requestUi.ru }, uk: { ...uk, ...requestUi.uk }, yi: { ...yi, ...requestUi.yi } };
