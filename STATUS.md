...
- ~~`/terms`, `/accessibility` and `/corrections` from the canonical site map are not built yet.~~ **Built 2026-08-24** (decision: all three are pre-launch requirements): `/terms` renders canonical Part XIV (purpose, user obligations, user materials, moderation), `/accessibility` renders canonical Part XV (IS 5568 / WCAG 2.0 AA intent, feature list, contact), `/corrections` renders the Part XIII user-rights list plus party right-of-reply; contact placeholder resolves to `PRIVACY_CONTACT_EMAIL`. All 7 locales have text; translations follow the same review gate.
- The "Initiator" section was removed from the About page in all six locales on 2026-07-28, so the operator is not named anywhere on the site. Part X of the canonical package lists it among the published items. **Decision (2026-08-24): omission accepted** — the section stays removed for launch; revisit only if legal review requires naming the operator.
