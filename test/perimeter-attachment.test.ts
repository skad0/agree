...
const productionBase = { NODE_ENV: "production", SESSION_SECRET: "secret", TRUSTED_PROXY: "cloudflare", TRUSTED_PROXY_SECRET: "edge-secret-012345678901234567890123", APP_BASE_URL: "https://example.org" };
...
test("production needs no owner contact and privacy policy preserves retention information", async () => {
  ...
  assert.deepEqual(loadConfig({ ...base, PRIVACY_CONTACT_EMAIL: "privacy@campaign.org" }), loadConfig(base));
  ...
  try {
    ...
    for (const locale of locales) {
      ...
      assert.doesNotMatch(html, /privacy@campaign\.org/);
      assert.doesNotMatch(await (await runtime.app.request(`/${locale}/methodology`)).text(), /privacy@campaign\.org/);
      assert.equal(t(locale, "privacyBody").split("\n\n").length, 6);
      assert.doesNotMatch(t(locale, "privacyBody"), /\{\{PRIVACY_CONTACT_EMAIL\}\}/);
