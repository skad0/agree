...
Owner-contact removal: removed all site-owner contact instructions from Privacy, Methodology and legacy stance copy in all seven locales. Removed the public contact setting from runtime configuration, environment templates and deployment requirements; existing deployment values are ignored. The public-copy regression checks cover 20 routes per locale, including a previously configured real-looking address, contact links and author metadata. Typecheck and all 116 tests pass. Privacy retention and self-service deletion remain available.
