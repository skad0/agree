...
Status: slices 1–5 merged; slice 6 + election-26 gates on PR #7 (`cursor_recipient-discovery-slice6-61d7`). **Target election confirmed: Knesset 26** (autumn 2026; evidence in `docs/ELECTION_TARGET.md`). Configurable default `electionTargetElectionNumber = 26`. CEC machine feed for 26 still blocked in the manifest; draft-only closed-list **artifact import** with **idempotent draft replace** (`import-artifact`) is the operator refresh path. Party-fallback send destinations wired from verified resolutions (no invented contacts). Finance contract still blocked. No production import activation, no unreviewed imported recipients published, no production stance seeds. Remaining gates: close CEC-26 + finance machine contracts; Anton supplies official closed-list JSON when available; accept/activate after review; parliamentary enrichment; participant UX/a11y and translation review.
