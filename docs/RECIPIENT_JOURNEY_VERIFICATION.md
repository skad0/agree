...
**Technical tests executed (automated, 2026-09-08):** typecheck; 107 Node tests covering directory search/suggest, signed basket tamper/expiry/limit, shared-mailbox and contact-proof invalidation, stance publish/authz/privacy (no `submitted_responses` on public cards), demand defaults/`?demand=` pre-check, enrichment matching/contacts/finance gates/job leases/pipeline blocked-source refusal; `npm run smoke` and `npm run smoke:pages`. Ordinary CI does not call live government sites. Stack PRs #2–#5 Node 22.13 and 24 green after abort-mock keepAlive fix.

**Technical tests still open:** load the directory while ETL and retention operate on the documented host budget; browser a11y/combobox screen-reader pass; production CEC reconciliation fixtures once the election is confirmed.
