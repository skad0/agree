...
Date: 2026-09-12. Status: slices 1–5 shipped; slice 6 adds publication-aware list/party enrichment (joint-list party only via reviewed membership), suggest `publicationId`, admin `/admin/directory` activate/rollback (accepted → active only), election banner when a publication is active, and automated backup/restore + no-store privacy checks. Party-fallback send destinations, live CEC/finance ingestion, browser a11y/combobox verification, load-on-host-budget, and participant UX remain open. Single-recipient `GET /:locale/request/build?recipient=` links still work; `?demand=` pre-checks that demand only.
...
**Technical tests executed (automated, 2026-09-12):** typecheck; 112 Node tests covering directory search/suggest (including `publicationId` when active), signed basket tamper/expiry/limit, shared-mailbox and contact-proof invalidation, stance publish/authz/privacy (no `submitted_responses` on public cards), demand defaults/`?demand=` pre-check, enrichment matching/contacts/finance gates/job leases/pipeline blocked-source refusal, publication activate/rollback, affiliation enrichment without inventing joint-list parties, backup/restore of election tables, absence of search-history/draft tables, admin directory CSRF/authz; `npm run smoke` and `npm run smoke:pages`. Ordinary CI does not call live government sites.

**Technical tests still open:** load the directory while ETL and retention operate on the documented host budget; browser a11y/combobox screen-reader pass; production CEC reconciliation fixtures once the election is confirmed; party-fallback destination handoff end-to-end.
